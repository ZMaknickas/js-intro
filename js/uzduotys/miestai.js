...
function kelione(greitis, laikas) {
    let maxA=0;
    let kelione = greitis*laikas;    
    for (let i=0; i<kelias.length; i+=2) 
    {
        if (kelione>=kelias[i+1] && kelione>maxA)
        {
        kelione=maxA;
        x=i;
        }
    }    
     console.log(kelias[x]);
}
kelione(80, 2)
